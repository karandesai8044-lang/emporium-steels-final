...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Inquiry / contact form submit — sends to WhatsApp
  document.querySelectorAll('form[data-inquiry]').forEach(form => {
    form.addEventListener('submit', function (e) {
      ...
      const material = form.querySelector('select')?.value || '';
      const qty = form.querySelectorAll('input[type="text"]')[1]?.value || '';
      const msg = form.querySelector('textarea')?.value || '';
      const text = `*New Enquiry — Emporium Steels*%0A%0A*Name:* ${encodeURIComponent(name)}%0A*Phone:* ${encodeURIComponent(phone)}%0A*Email:* ${encodeURIComponent(email)}%0A*Material:* ${encodeURIComponent(material)}%0A*Quantity:* ${encodeURIComponent(qty)}%0A*Message:* ${encodeURIComponent(msg)}`;
      window.open('https://wa.me/918780975192?text=' + text, '_blank');
      setTimeout(() => {
        btn.textContent = 'Enquiry Sent ✓';
        form.reset();
        setTimeout(() => { btn.textContent = 'Submit Enquiry'; }, 3000);
      }, 800);
    });
  });
